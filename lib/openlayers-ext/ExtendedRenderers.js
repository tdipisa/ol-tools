...
OpenLayers.Renderer.SVGExtended = OpenLayers.Class(OpenLayers.Renderer.SVG, {
	...
	LABEL_ID_SUFFIX_HTML: "_htmlLabel",
    ...
    removeText: function(featureId) {
		...
		label = document.getElementById(featureId + this.LABEL_ID_SUFFIX + "_htmlLabel");
        if (label) {
    ...
    drawText: function(featureId, style, location) {
		if(style.labelType == 1){			
			var resolution = this.getResolution();

			var x = ((location.x - this.featureDx) / resolution + this.left);
			var y = (location.y / resolution - this.top);

			var suffix = this.LABEL_ID_SUFFIX + this.LABEL_ID_SUFFIX_HTML;
			var label = this.nodeFactory(featureId + suffix, "foreignObject");

			label.setAttributeNS(null, "x", x);
			label.setAttributeNS(null, "y", -y);
			
			var value = style.label;
			
			var container = "<div id=\"" + featureId + suffix + "_html\"></div>";
			label.innerHTML = container;
			
			label.childNodes[0].innerHTML = value;
			
			this.root.appendChild(label);
			
			label.setAttributeNS(null, "width", label.childNodes[0].clientWidth + 200);   //TODO: Fix this 
			label.setAttributeNS(null, "height", label.childNodes[0].clientHeight + 100); //TODO: Fix this 			
		}else{
			return OpenLayers.Renderer.SVG.prototype.drawText.apply(this, arguments);
		}
    }	
});
...
OpenLayers.Renderer.CanvasExtended = OpenLayers.Class(OpenLayers.Renderer.Canvas, {
    ...
    drawText: function(location, style) {
		if(style.labelType == 1){
			...
			var value = style.label;
			var container = "<div xmlns='http://www.w3.org/1999/xhtml'>" + value + "</div>";
			
			var data = "<svg xmlns='http://www.w3.org/2000/svg' viewBox=\"0 0 1024 748\" style=\"display: block;\">" +
						 "<foreignObject" + " x=\"" + pt[0] + "\"" + " y=\"" + pt[1] + "\"" + " width=\"1024\" height=\"748\">" +
						   container +
						 "</foreignObject>" +
					   "</svg>";
			...
			var ctx = this.canvas;			
			img.onload = function() { 
				ctx.drawImage(img, pt[0], pt[1]); 
			}	
			img.src = url;			
		}else{
			return OpenLayers.Renderer.Canvas.prototype.drawText.apply(this, arguments);
		}        
    }
});
